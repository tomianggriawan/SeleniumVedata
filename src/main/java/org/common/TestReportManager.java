...
public class TestReportManager {
    ...
    /**
     * Mencatat kegagalan yang disebabkan oleh error jaringan (HTTP 4xx/5xx atau JS error),
     * menyematkan blok HTML detail dari {@link NetworkEventAnalyzer.AnalysisResult}
     * langsung ke dalam step tabel HTML report.
     *
     * @param message       Pesan ringkas kegagalan untuk label step
     * @param analysisResult Hasil analisis dari NetworkEventAnalyzer.analyze()
     */
    public void logNetworkFail(String message, NetworkEventAnalyzer.AnalysisResult analysisResult) {
        if (currentTest != null) {
            currentTest.setStatus("FAIL");
            String htmlDetail = analysisResult != null ? analysisResult.buildHtmlReport() : "";
            currentTest.addStepWithHtml("FAIL", message, htmlDetail);
            currentTest.setError(message, null);
            System.err.println("  🔴 [NETWORK FAIL] " + message);
            if (analysisResult != null) {
                for (NetworkEventAnalyzer.NetworkError e : analysisResult.getErrors()) {
    ...
    public void generateHtmlReport(String filePath) {
        ...
        for (TestCase tc : testCases) {
            ...
            for (TestStep step : tc.getSteps()) {
                ...
                html.append("            <tr>\n")
                    .append("              <td style=\"color: var(--text-muted);\"").append(">")
                    .append(step.timestamp.format(TIME_FORMAT)).append("</td>\n")
                    .append("              <td><span class=\"step-status ")
                    .append(stepStatusClass).append("\">")
                    .append(step.status).append("</span></td>\n")
                    .append("              <td>")
                    .append(step.description);
                // Jika step memiliki HTML detail tambahan (mis. network error block), sisipkan
                if (step.htmlDetail != null && !step.htmlDetail.isEmpty()) {
                    html.append(step.htmlDetail);
                }
                html.append("</td>\n")
                    .append("            </tr>\n");
            }
    ...
    private static class TestCase {
        ...
        public void addStepWithHtml(String status, String description, String htmlDetail) {
            steps.add(new TestStep(status, description, htmlDetail));
        }
    ...
    private static class TestStep {
        ...
        private final String description;
        private final String htmlDetail; // Opsional: blok HTML tambahan (mis. network error detail)
        ...
        public TestStep(String status, String description, String htmlDetail) {
            this.timestamp  = LocalDateTime.now();
            this.status     = status;
            this.description = description;
